...
public class DeltaSetCalculator
{
  ...
  /*
   * A hibernate-specific calculation. Uses the values passed to the Hibernate Interceptor.onFlushDirty() to perform the calculation.
   * 
   * @param propertyNames A string array of all the property names passed in to the obFlushDirty method. @param previousState The Object
   * array representing the previous state of the properties named in the propertyNames array. @param currentState The Object array
   * representing the current state of the properties named in the propertyNames array. @return The DeltaSet representing the changes
   * encountered in the property states.
   */
  public static DeltaSet calculateDeltaSet(SessionFactory sf, Set<String> validPropertyNames, Set<String> invalidPropertyNames,
      Serializable entityId, Object entity, String[] propertyNames, Object[] previousState, Object[] currentState)
  {
    ...
    try {
      checkProperties(entity, sf, validPropertyNames, invalidPropertyNames, propertyNames, previousState, currentState, deltaSet);
    } catch (Throwable t) {
   ...
   * @param deltaSet
   */
  private static void checkProperties(Object entity, SessionFactory sf, Set<String> validPropertyNames, Set<String> invalidPropertyNames,
      String[] propertyNames, Object[] previousState, Object[] currentState, DeltaSet deltaSet)
  {
    ...
    for (int i = 0; i < propertyNames.length; i++) {
      ...
      if (Hibernate.isInitialized(propertyPreviousState) || Hibernate.isInitialized(propertyCurrentState)) {
        final PropertyDelta delta = getDeltaOrNull(entity, sf, propertyNames[i], propertyType, propertyPreviousState, propertyCurrentState);
        if (delta != null) {
   ...
   */
  public static DeltaSet calculateDeltaSet(Object entity, SessionFactory sf, Serializable entityId, Class< ? > entityType, Object obj1,
      Object obj2)
  {
  ...
  public static PropertyDelta getDeltaOrNull(Object entity, SessionFactory sf, String propertyName, Class< ? > propertyType,
      Object oldValue, Object newValue)
  {
    ...
    try {
      if (Collection.class.isAssignableFrom(propertyType)) {
        ...
        delta = getCollectionDelta(entity, (SessionFactoryImplementor) sf, propertyName, propertyType, oldValue, newValue, delta);
   ...
   * @return
   */
  private static PropertyDelta getArrayDelta(SessionFactory factory, String propertyName, Class< ? > propertyType, Object oldValue,
      ...
      Class propertyType, Object oldValue, Object newValue, PropertyDelta delta)
  {
    ...
    if (Hibernate.isInitialized(oldCollectionValue) && Hibernate.isInitialized(newCollectionValue)) {
      ...
      try {
        CollectionType propertyType2 = (CollectionType) factory.getClassMetadata(entity.getClass()).getPropertyType(propertyName);
      ...
      collectionDelta = new CollectionPropertyDelta(propertyName, returnedClass, oldCollectionValue, newCollectionValue);
      if (collectionDelta.anyChangeDetected()) {
        delta = collectionDelta;
      }
      collectionDelta = null;
    ...
    try {
      ...
      if (clazz != null) {
        if (clazz.hasIdentifierProperty() == true) {
          if (clazz.getIdentifier(obj1, EntityMode.POJO).equals(clazz.getIdentifier(obj2, EntityMode.POJO)) == true) {
            return true;
          }
        }
      }
