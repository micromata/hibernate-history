...
public class DefaultHistoryFormatter implements HistoryFormatter
{
  ...
  /**
   * Hier werden die keys gespeichert, die nicht gefunden werden konnte. Dies soll ein erneutes Suchen und eine damit verbundene
   * MissingResourceException verhindern. Das Keys nicht gefunden werden, ist normal.
   */
  ...
  /**
   * <b>Beachte Sicherheitshinweis:</b> Beim Überladen bitte dynamische Inhalte immer mit escapeHtml ausgeben (Vorsicht: Cross site
   * scripting).
   ...
   *      de.micromata.hibernate.history.HistoryEntry)
   */
  public String formatTimestamp(Session session, final Locale locale, Object changed, HistoryEntry historyEntry, PropertyDelta delta)
  {
    return sdf.format(historyEntry.getTimestamp());
  }
   ...
   *      de.micromata.hibernate.history.HistoryEntry)
   */
  public String formatAction(Session session, final Locale locale, Object changed, HistoryEntry historyEntry, PropertyDelta delta)
  {
  ...
  /**
   * Sucht zuerst im ResourceBundle nach einem Eintrag class.property.xxx (z. B. de.micromata.printing.vw.VWUserDO.property.commitAGBDate),
   * dann nach common.property.xxx und falls beides nicht gefunden wird der Property-Name direkt mit einem Großbuchstaben beginnen
   * ausgegeben. (don't forget escapeHtml!)
   * @see de.micromata.hibernate.history.web.HistoryFormatter#formatProperty(org.hibernate.Session, java.util.Locale, java.lang.Object,
   *      de.micromata.hibernate.history.HistoryEntry, de.micromata.hibernate.history.delta.PropertyDelta)
   */
  public String formatProperty(Session session, final Locale locale, Object changed, HistoryEntry historyEntry, PropertyDelta delta)
  {
    if (delta == null) {
      return "";
    }
  ...
  /**
   * Wird von formatOldValue und formatNewValue aufgerufen. Dies kann überladen werden, wenn der alte und neue Wert gleichermaßen formatiert
   * werden sollen. Macht hier ersteinmal nichts außer ein escapeHtml(value) aufzurufen.
   * 
   * <b>Beachte Sicherheitshinweis:</b> Beim Überladen bitte dynamische Inhalte immer mit escapeHtml ausgeben (Vorsicht: Cross site
   * scripting).
   * @param session
   ...
   * @see de.micromata.hibernate.history.web.HistoryFormatter#formatOldValue(org.hibernate.Session, java.util.Locale, java.lang.Object,
   *      de.micromata.hibernate.history.HistoryEntry, de.micromata.hibernate.history.delta.PropertyDelta)
   */
  public String formatOldValue(Session session, final Locale locale, Object changed, HistoryEntry historyEntry, PropertyDelta delta)
  {
    return asString(session, locale, historyEntry.getClassName(), delta == null ? "" : delta.getPropertyName(), delta == null ? "" : delta.getOldValue());
  }

  /**
   * (don't forget escapeHtml!)
   * @see de.micromata.hibernate.history.web.HistoryFormatter#formatNewValue(org.hibernate.Session, java.util.Locale, java.lang.Object,
   *      de.micromata.hibernate.history.HistoryEntry, de.micromata.hibernate.history.delta.PropertyDelta)
   */
  public String formatNewValue(Session session, final Locale locale, Object changed, HistoryEntry historyEntry, PropertyDelta delta)
  {
    return asString(session, locale, historyEntry.getClassName(), delta == null ? "" : delta.getPropertyName(), delta == null ? "" : delta.getNewValue());
  }

  /**
   * @see de.micromata.hibernate.history.web.HistoryFormatter#isVisible(org.hibernate.Session, java.util.Locale, java.lang.Object,
   *      de.micromata.hibernate.history.HistoryEntry, de.micromata.hibernate.history.delta.PropertyDelta)
   */
  public boolean isVisible(Session session, final Locale locale, Object changed, HistoryEntry historyEntry, PropertyDelta delta)
  {
    return true;
  }
}
